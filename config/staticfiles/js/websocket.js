document.addEventListener("DOMContentLoaded", function () {
    ...
    let chart;
    let selectedCurrency = "BTC/USDT";  // Начальная валюта
    const maxDataPoints = 50;
    let startTime = Date.now(); // Время открытия сайта
        ...
        try {
            ...
            if (data.type === "send_price_update") {
                ...
                historyData[data.symbol].push({
                    ...
                    timeMs: Date.now() - startTime // Для отслеживания времени с начала открытия сайта
                });
    ...
    function smoothData(data) {
        // Использование простого скользящего среднего для сглаживания данных
        return data.map((value, index, arr) => {
            if (index === 0 || index === arr.length - 1) return value;
            // Среднее значение среди соседних точек
            return (arr[index - 1] + value + arr[index + 1]) / 3;
        });
    }

    // Добавление обработчиков событий для наведения и клика на валюту
